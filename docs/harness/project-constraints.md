...
| `DOCS-001` | `docs` | `.agents/plans/` 跟踪主模板和实现型 exemplar；真实计划和运行态计划文件默认不提交 | `.gitignore` / `AGENTS.md` | git ignore / review | `git check-ignore -q .agents/plans/EXAMPLE-implementation.md && exit 1 || true` | `partial` | `maintenance_candidate` | `.agents/plans/TEMPLATE.md` 与 `.agents/plans/EXAMPLE-implementation.md` 是默认提交的 plan 目录 markdown |
