...
`check-shard` 不需要独立的 SDK 主流程。CLI 使用 `RequireShardedCluster=true` 保留拓扑校验，获取完整集合统计后由 formatter 默认展示尚未分片的集合；`ShardedOnly` 只表达 SDK 结果过滤，不再同时承担拓扑校验和 CLI 展示语义。
...
- `RequireShardedCluster=true` 时，非分片集群返回 `ErrNotSharded`。
- `ShardedOnly=true` 时只返回已分片集合。
...
- MongoDB 3.4 等 profiler 不提供 `queryHash` 时，`QueryHash` 返回 `legacy:` 前缀的兼容标识；它由 namespace、operation 和 plan summary 稳定生成，Detail 返回该聚合组最新的匹配文档，不承诺等同于新版查询形状哈希。
...
如果 SDK 的业务查询继续使用 `context.Background()`，外部调用方无法取消长操作，也无法统一超时。失败连接、派生成员连接和 cursor 的资源释放应改用 `context.WithoutCancel(ctx)` 派生的短超时 cleanup context，避免业务 ctx 取消后反而跳过清理。
